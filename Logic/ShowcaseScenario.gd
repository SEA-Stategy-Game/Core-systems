...
func _execute_local_demo_command(command: Dictionary) -> bool:
	...
	match String(command.get("action", "")):
		"CHOP_AND_RETURN":
			return _gateway.get_node_or_null("/root/ActionGateway").go_chop_tree_and_return(
				int(command.get("unit_id", -1)),
				int(command.get("target_id", -1)),
				int(command.get("player_id", -1))
			)
		"MOVE":
			var pos_dict: Dictionary = command.get("target", {})
			return _gateway.get_node_or_null("/root/ActionGateway").move_unit(
				int(command.get("unit_id", -1)),
				Vector2(float(pos_dict.get("x", 0.0)), float(pos_dict.get("y", 0.0))),
				int(command.get("player_id", -1))
			)
		"ATTACK":
			return _gateway.get_node_or_null("/root/ActionGateway").attack_target(
				int(command.get("unit_id", -1)),
				int(command.get("target_id", -1)),
				int(command.get("player_id", -1))
			)
...
func _on_authoritative_state_applied(state: Dictionary) -> void:
	...
	if _demo_active and not _authoritative_signature.is_empty() and not _local_signature.is_empty():
		if _local_signature == _authoritative_signature:
			_demo_phase = "synced"
		else:
			_demo_phase = "desynced"
	_emit_state_change()
...
func _update_local_signature() -> void:
	var snapshot := _build_local_snapshot()
	_local_signature = DeterminismHash.snapshot_signature(snapshot)
