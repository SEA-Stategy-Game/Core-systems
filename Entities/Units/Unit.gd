...
func sync_from_snapshot(snapshot: Dictionary) -> void:
	...
	if snapshot.has("position"):
		var new_pos := Vector2(snapshot["position"]["x"], snapshot["position"]["y"])
		if global_position.distance_squared_to(new_pos) > 1.0:
...
func get_attack_range_radius() -> float:
	var range_area := attack_range_area
	if range_area == null:
		return 60.0
	var collision_shape := range_area.get_node_or_null("CollisionShape2D")
	...
	var sx: float = abs(scale.x) * abs(range_area.scale.x) * abs(cs.scale.x)
	var sy: float = abs(scale.y) * abs(range_area.scale.y) * abs(cs.scale.y)
...
func take_damage(amount: int) -> void:
	...
	var sprite_node = get_node_or_null("Arthax")
	if not sprite_node:
		sprite_node = get_node_or_null("Sprite2D")
	if sprite_node:
		if sprite_node.material:
			sprite_node.material = sprite_node.material.duplicate(true)
		var tween = get_tree().create_tween()
		sprite_node.modulate = Color(1, 0, 0)
		tween.tween_property(sprite_node, "modulate", Color(1, 1, 1), 0.2)
	if current_health <= 0:
		die()
...
func _on_navigation_agent_2d_velocity_computed(safe_velocity: Vector2) -> void:
	var peer := multiplayer.multiplayer_peer
	if peer != null and peer.get_connection_status() == MultiplayerPeer.CONNECTION_CONNECTED and not multiplayer.is_server():
		return
